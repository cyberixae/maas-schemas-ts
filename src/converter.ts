...
export function fromFile(
  ...
  */
  ...
  // eslint-disable-next-line
      ...
      fs.appendFileSync(fd, `import * as ${iname} from 'src/${bobobo3}';`);
      return [
        ref.description,
        gen.typeDeclaration(
          capitalize(k),
...
fs.appendFileSync(fd, '');
fs.appendFileSync(fd, schema.description);
fs.appendFileSync(fd, '');
fs.appendFileSync(fd, '!!! AUTO GENERATED BY CONVERT.TS REFRAIN FROM MANUAL EDITING !!!');
fs.appendFileSync(fd, '');
fs.appendFileSync(fd, '*/');
fs.appendFileSync(fd, '');
fs.appendFileSync(fd, "import * as t from 'io-ts';");

const declarations = fromFile(schema as JSONSchemaFile);
fs.appendFileSync(fd, '');
fs.appendFileSync(fd, `export const schemaId = '${schema.$id}';`);
...
for (const [c, d] of declarations) {
  ...
  fs.appendFileSync(fd, gen.printStatic(d));
  fs.appendFileSync(fd, gen.printRuntime(d));
}
