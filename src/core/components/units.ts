...
export const Uuid = t.brand(t.string, (x): x is t.Branded<string, UuidBrand> => ( typeof x !== 'string' || x.match("^[A-Fa-f0-9]{8}(-[A-Fa-f0-9]{4}){3}-[A-Fa-f0-9]{12}$") !== null ), 'Uuid')
...
export const Url = t.brand(t.string, (x): x is t.Branded<string, UrlBrand> => ( typeof x !== 'string' || x.match("^(https?|ftp):\\/\\/[^\\s/$.?#].[^\\s]*$") !== null ), 'Url')
...
export const Arn = t.brand(t.string, (x): x is t.Branded<string, ArnBrand> => ( typeof x !== 'string' || x.match("^arn:aws:[a-z]+:[\\w\\-]*:\\d{12}:[ \\w\\-]+(\\/[\\w\\- \\/]+)*") !== null ) && ( typeof x !== 'string' || x.length >= 1 ) && ( typeof x !== 'string' || x.length <= 256 ), 'Arn')
...
export const IdentityId = t.brand(t.string, (x): x is t.Branded<string, IdentityIdBrand> => ( typeof x !== 'string' || x.match("^[aepus]{2}-[\\w]{4}-\\d:[a-f\\d]{8}(-[a-f\\d]{4}){3}-[a-f\\d]{12}$") !== null ), 'IdentityId')
...
export const Currency = t.brand(t.intersection([
  ...
  | 'GBP'
  | 'SGD'
  | 'USD', CurrencyBrand> => true, 'Currency')
...
export const IsoDate = t.brand(t.string, (x): x is t.Branded<string, IsoDateBrand> => ( typeof x !== 'string' || x.match("^\\d{4}-\\d{2}-\\d{2}") !== null ), 'IsoDate')
