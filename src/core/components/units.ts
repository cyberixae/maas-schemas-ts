...
export const Uuid = t.brand(t.string, (x): x is t.Branded<string, UuidBrand> => x === x && (x.match(/^[A-Fa-f0-9]{8}(-[A-Fa-f0-9]{4}){3}-[A-Fa-f0-9]{12}$/) !== null), 'Uuid')
interface UuidBrand {
...
export const Url = t.brand(t.string, (x): x is t.Branded<string, UrlBrand> => x === x && (x.match(/^(https?|ftp):\/\/[^\s/$.?#].[^\s]*$/) !== null), 'Url')
interface UrlBrand {
  readonly Url: unique symbol
}

export type Arn = t.Branded<string, ArnBrand>
export const Arn = t.brand(t.string, (x): x is t.Branded<string, ArnBrand> => x === x && (x.match(/^arn:aws:[a-z]+:[\w\-]*:\d{12}:[ \w\-]+(\/[\w\- \/]+)*/) !== null) && (x.length >= 1) && (x.length <= 256), 'Arn')
interface ArnBrand {
  readonly Arn: unique symbol
}

export type IdentityId = t.Branded<string, IdentityIdBrand>
export const IdentityId = t.brand(t.string, (x): x is t.Branded<string, IdentityIdBrand> => x === x && (x.match(/^[aepus]{2}-[\w]{4}-\d:[a-f\d]{8}(-[a-f\d]{4}){3}-[a-f\d]{12}$/) !== null), 'IdentityId')
...
export const Currency = t.brand(t.string, (x): x is t.Branded<string, CurrencyBrand> => x === x, 'Currency')
...
export const Time = t.brand(t.number, (x): x is t.Branded<number, TimeBrand> => x === x && (x >= 1451606400) && (x <= 9007199254740991) && (Number.isInteger(x)), 'Time')
interface TimeBrand {
...
export const Duration = t.brand(t.number, (x): x is t.Branded<number, DurationBrand> => x === x && (x >= -9007199254740991) && (x <= 9007199254740991) && (Number.isInteger(x)), 'Duration')
...
export const IsoDate = t.brand(t.string, (x): x is t.Branded<string, IsoDateBrand> => x === x && (x.match(/^\d{4}-\d{2}-\d{2}/) !== null), 'IsoDate')
